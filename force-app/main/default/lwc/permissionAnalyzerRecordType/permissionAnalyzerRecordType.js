...
export default class PermissionAnalyzerRecordType extends LightningElement {
    ...
    set permSetLists(value) {
        ...
        if(this.recordTypeCache[this.objectName])
        {
            this.showSpinner = false;
            this.recordTypes = JSON.parse(JSON.stringify(this.recordTypeCache[this.objectName]));
            this.showRecordType = this.recordTypes.length > 0 ? true : false;
            console.log('OUTPUT : recordTypes',JSON.stringify(this.recordTypes));
        }
        else
        {
    ...
    handleSearchKeyChange(event) {
        ...
        this.hasRecType = this.filteredRecType.length > 0 ? true : false;
    }
