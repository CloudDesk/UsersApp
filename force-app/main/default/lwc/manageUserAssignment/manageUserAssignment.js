...
export default class ManageUserAssignment extends LightningElement {
    ...
    handleSave() {
        ...
        assignPermissionsAndGroups({
                ...
                publicGroupIds: this.publicGroupIds,
                queueIds: this.queueIds
            })
            .then(result => {
                this.showToast('Success', 'Assignment saved successfully to the user', 'success');
                this.closeModal();
                this.showSpinner = false;
                return Promise.all([
                    refreshApex(this.wiredResult),
                    refreshApex(this.wiredSelectedResult)
                ]);
            })
            .catch((error) => {
                ...
                this.showToast('Error', extractedMessage, 'error');
                this.closeModal();
                this.showSpinner = false;
            });
    }
